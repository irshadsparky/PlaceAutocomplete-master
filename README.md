...
[![Build Status](https://raw.githubusercontent.com/irshadsparky/PlacesAutocomplete/master/resources/autocomplete.gif)](https://travis-ci.org/seatgeek/android-PlacesAutocompleteTextView)
...
The `PlacesAutocomplete` is available from the sonatype snapshots repository.
...
dependencies {
    implementation 'com.github.irshadsparky:PlacesAutocomplete:master-SNAPSHOT'
}
```
...
3. With your API key, you're ready to add the `PlacesAutocomplete` to your layout xml:
...
_Note: you can treat the `PlacesAutocomplete` the same as any `AutocompleteTextView`
...
`PlacesAutocomplete` provides a helper method `getDetailsFor(Place, DetailsCallback)` that
...
grab the instance of the `PlacesApi` from `PlacesAutocomplete#getApi()` and manage the
...
The `PlacesAutocomplete` is styleable using the `pacv_placesAutoCompleteTextViewStyle`
...
By default, the `PlacesAutocomplete` will bias the results by a geoip lookup of
...
`Location` into the `PlacesAutocomplete` using the `#setCurrentLocation()` method.
